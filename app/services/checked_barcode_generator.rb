class CheckedBarcodeGenerator
  ...
  def initialize(unchecked_isbn_barcode)
    @unchecked_isbn_barcode = unchecked_isbn_barcode.to_s # cast as string in case it isn't already
  end
  ...
  def generate_check_digit
    digits = unchecked_isbn_barcode.chars.map(&:to_i) # Split
    multiplied_digits = digits.map.with_index do |digit, index|
      index.even? ? digit : digit * 3 # Multiply odd index
    end
    summed_digits = multiplied_digits.sum # sum
    modulo = summed_digits % 10 # modulo
    10 - modulo
  end

  def join_check_digit(check_digit)
    (unchecked_isbn_barcode + check_digit.to_s).to_i
  end
end
